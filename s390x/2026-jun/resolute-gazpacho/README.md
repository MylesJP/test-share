# Sunbeam Gazpacho validation on Resolute/s390x

This directory contains the validation report for a single-LPAR Sunbeam
OpenStack Gazpacho deployment on Ubuntu Resolute/s390x.
...
- Ubuntu series: `resolute`
- OpenStack series: `gazpacho`
- Architecture: `s390x`
- Run ID: `20260626T150509Z`
- Hypervisor: `special19`, QEMU, `up`
- Guest image: Ubuntu 24.04 s390x cloud image
- Guest boot: passed
- Floating-IP SSH: passed
- Cinder volume create/attach: passed
- Tempest smoke: `Ran: 135`, `Passed: 98`, `Skipped: 37`, `Failed: 0`
...
Watcher, VPNaaS, QoS, Octavia traffic validation, and multinode scenarios.
...
- `juju_status.txt` shows `cinder-router` app-level `waiting` on a database
  ...
  validation passed.
- Resolute's distro Tempest plugin set exposed a Python 3.14 compatibility issue
  during discovery. A Tempest-only compatibility shim was used for this run.
...
Resolute/s390x using the Kubernetes-charm-based Sunbeam architecture. It is not
a validation of optional OpenStack services that were not deployed.
